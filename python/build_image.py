"""Build the Python tagtest Docker image on the HMI.  Normalizes
tar ownership to 0:0 to avoid Windows UID metadata tripping
rootless Docker's lchown.

Outputs the bpclient-python-tagtest:dev image.  After this completes,
run e.g.
...
with tarfile.open(fileobj=buf, mode='w:gz') as tf:
    for root, dirs, files in os.walk(here):
        # skip build / cache / vcs / virtualenv artifacts
        dirs[:] = [d for d in dirs if d not in (
            'build', '.git', '__pycache__', '.pytest_cache',
            'dist', '*.egg-info', '.venv', 'venv',
        )]
        for name in files:
...
url = 'http://10.0.0.166:2375/build?t=bpclient-python-tagtest:dev&dockerfile=Dockerfile&rm=1'
